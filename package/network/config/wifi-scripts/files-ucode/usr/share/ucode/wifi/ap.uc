...
function iface_auth_type(config, band) {
	...
	append_vars(config, [
		...
		'radius_das_client', 'radius_das_port', 'owe_groups', 'owe_ptk_workaround', 'own_ip_addr', 'dynamic_own_ip_addr', 'radius_client_addr',
...
function iface_ppsk(config) {
	if (!(config.auth_type in [ 'none', 'owe', 'psk', 'sae', 'psk-sae', 'psk-sae-compat', 'wep' ]) || !config.auth_server_addr || config.macfilter == 'radius')
		return;
